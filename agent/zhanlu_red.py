    ...
    def step(self, sim_time,obs_red,**kwargs):
        ...
        if self.agent_state == 0:
            ...
            for ship in obs_red['units']:
                if ship['LX'] == 21:
                    if index == 1:
                        cmd_list.extend(self._ship_movedeploy(ship['ID'], SHIP_POINT1))  # extend() 函数用于在列表末尾一次性追加另一个序列中的多个值（用新列表扩展原来的列表）。
                        print('1号护卫舰就位')
                        index += 1
                        continue
                    if index == 2:
                        cmd_list.extend(self._ship_movedeploy(ship['ID'], SHIP_POINT2))  # extend() 函数用于在列表末尾一次性追加另一个序列中的多个值（用新列表扩展原来的列表）。
                        print('2号护卫舰就位')
                        index += 1
                        continue
            self.agent_state = 1
        ...
        # 空战歼击机2机编队--JK
        if self.agent_state == 17 and sim_time>4000:
            print("step-第三波次!")
