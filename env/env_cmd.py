...
def validity_area(pos_x, pos_y, length, width, direction):
    ...
    # 矩形区域左下角坐标
    ld_x = pos_x - 0.5*length*math.cos(direction) - 0.5*width*math.sin(direction)
    ld_y = pos_y - 0.5*length*math.sin(direction) + 0.5*width*math.cos(direction)
    # 矩形区域右下角坐标
    rd_x = pos_x - 0.5*length*math.cos(direction) + 0.5*width*math.sin(direction)
    rd_y = pos_y - 0.5*length*math.sin(direction) - 0.5*width*math.cos(direction)
    # 矩形区域左上角坐标
    lt_x = pos_x + 0.5*length*math.cos(direction) - 0.5*width*math.sin(direction)
    lt_y = pos_y + 0.5*length*math.sin(direction) + 0.5*width*math.cos(direction)
    # 矩形区域右上角坐标
    rt_x = pos_x + 0.5*length*math.cos(direction) + 0.5*width*math.sin(direction)
    rt_y = pos_y + 0.5*length*math.sin(direction) - 0.5*width*math.cos(direction)
    # 越界检查与t修正
    diag = math.sqrt(length**2 + width**2)  # 矩形对角线长度
    if ld_x < X_Min or rt_x > X_Max or lt_x < X_Min or rd_x > X_Max:
        # 左下角x坐标越界, 右上角x坐标越界, 对应的theta区间(0, 90]
        # 左上角x坐标越界, 或者右下角x坐标越界, 对应theta区间[90, 180)
        # 这四种是x轴坐标越界情况
        # 中心点距离最近的x轴边界距离(并检验中心点是否合理(距离边界至少2公里, 否则没法转弯或者一直大角度机动))
        margin = min(pos_x-X_Min, X_Max-pos_x)
        assert margin >= 1000
        # 矩形中心点到越界顶点距离在x轴上的投影长度proj为
        proj = 0.5*diag*math.sin(direction+theta) if direction <= 90 else 0.5*diag*math.sin(180-direction+theta)
        # 确定长宽缩小比例
        ratio = margin / proj
        length = length * ratio
        width = width * ratio
        ...
        # 越界检查与t修正
        diag = math.sqrt(length ** 2 + width ** 2)  # 矩形对角线长度
    if lt_y < Y_Min or rd_y > Y_Max or rt_y < Y_Min or ld_y > Y_Max:
        # 左上角y坐标越界, 或者右下角y坐标越界, 对应theta区间为(0, 90]
        # 右上角y坐标越界, 或者左下角y坐标越界, 对应theta区间[90, 180)
        # 这四种是y轴坐标越界情况
        # 中心点距离最近的y轴边界距离(并检验中心点是否合理(距离边界至少2公里, 否则没法转弯或者一直大角度机动))
        margin = min(pos_y-Y_Min, Y_Max-pos_y)
        assert margin >= 1000
        # 矩形中心点到越界顶点距离在y轴上的投影长度proj为
        proj = 0.5*diag*math.cos(direction+theta) if direction <= 90 else 0.5*diag*math.cos(180-direction+theta)
        # 确定长宽缩小比例
        ratio = margin / proj
        length = length * ratio
        width = width * ratio
